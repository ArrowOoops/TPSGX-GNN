...
def load_cora(num_feat, filepathcites, filepathcontent):
    num_feats = num_feat
    num_nodes = 2708
...
if __name__ == '__main__':
    ...
    while cnt < 5:
        ...
        for i in range(10):
            cur = 143 * (i + 1)
            string = "test" + str(i + 1) + "vertical_" + str(cur)+"features content"
            filepathcites = "Vertical_Cora/cora.cites"
            filepathcontent = "Vertical_Cora/" + string

            dataset = load_cora(cur, filepathcites, filepathcontent)
            device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
            model = Net_SAGE(cur, 7).to(device)
        ...
        for i in range(10):
            ...
            dataset = load_cora(cur, filepathcites, filepathcontent)
            device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
            model = Net_TPSGX(cur, 7).to(device)
